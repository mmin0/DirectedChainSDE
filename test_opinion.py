...
def main(
    ...
    test_t_size = 2
    test_ts, data_size, test_dataloader = data.StochasticOpinion_data(batch_size=batch_size, 
                                                 device=device, 
                                                 t_size=test_t_size, 
                                                 phase='test',
    ...
    averaged_generator.load_state_dict(torch.load("opinion_generator.net"))
    generator.load_state_dict(averaged_generator.module.state_dict())
    
    
    def initial_generator(size, device):
        return torch.rand(size, device=device).unsqueeze(-1) * 4 - 2
    
    
    
    utils.plot(test_ts, generator, 
               test_dataloader, 
               num_plot_samples, 
               plot_locs, 
               initial_generator,
               propagations=propagations,
               addr=sys.path[0]+"/fig/StochasticOpinion")
    
    sig_mmds = []
    for i in range(10):
        initial = initial_generator(batch_size, device)
        sig_mmd = utils.evaluate_sig_loss(test_ts, batch_size, test_dataloader, generator, initial, sig_depth=8, propagations=propagations)
